...
do
    for i2 in `seq 5 9` 
    do
	echo benchmark B $i1 vs benchmark B $i2
	game/play_match.pl game game/holdem.nolimit.2p.game 3000 0 ${benchmarks[i1]} neuralnet/play_nolimit_agent.sh ${benchmarks[i2]} neuralnet/play_nolimit_agent.sh
    done
done
