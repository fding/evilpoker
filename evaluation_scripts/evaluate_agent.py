...
def play_game(aid, benchmarkfile, nn_agent_file, gamefile, ngames):    
    game_results = defaultdict(list)

    btes = map(ord, os.urandom(2))
    match_args = (gamefile, btes[0] * 256 + btes[1], "benchmark", benchmarkfile, aid, nn_agent_file)

    print match_args
    # play the games and record the output (which is the scores of the agents in the game)
    for _ in xrange(ngames):
        output = subprocess.check_output("game/play_match.pl game %s 1000 %d %s %s %s %s" % match_args, shell=True)
...
parser.add_argument('--nn_agent_file', dest='nn_agent_file', type=str, default='neural net agent playing script')
args = parser.parse_args()

play_game(args.aid, args.bmfile, args.nn_agent_file, args.gamefile, args.ngames)
